...
class VarManager : public TObject
{
 ...
 public:
  enum Variables {
    ...
    kMCMotherPdgCode,
    kMCCosThetaHE, // Cosine of helicity angle
    kMCPhiHE,      // Phi angle in helicity frame
    kMCPhiTildeHE, // Tilde phi angle in helicity frame
    kMCCosThetaCS, // Cosine of Collins-Soper angle
    kMCPhiCS,      // Phi angle in Collins-Soper frame
    kMCPhiTildeCS, // Tilde phi angle in Collins-Soper frame
    kMCCosThetaPP, // Cosine of the angle between the pair momentum and the proton direction
    kMCCosThetaRM, // Cosine of the angle between the pair momentum and the direction of the reconstructed J/psi
...
void VarManager::FillPair(T1 const& t1, T2 const& t2, float* values)
{
  ...
  bool usePP = fgUsedVars[kCosThetaPP] || fgUsedVars[kPhiPP]; // production plane frame
...
void VarManager::FillPairVn(T1 const& t1, T2 const& t2, float* values)
{
  ...
  bool useGlobalPolarizatiobSpinOne = fgUsedVars[kCosThetaStarA] || fgUsedVars[kCosThetaStarB] || fgUsedVars[kCosThetaStarC];
